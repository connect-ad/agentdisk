...
function stubOutbound(options: StubOptions = {}): {
  mailerSendBodies: unknown[];
  identityBodies: unknown[];
  tokenCalls: number;
} {
  const { identity = "link", mail = "accepted" } = options;
  const mailerSendBodies: unknown[] = [];
  const identityBodies: unknown[] = [];
  let tokenCalls = 0;

  const recorded = { mailerSendBodies, identityBodies, get tokenCalls() { return tokenCalls; } };
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.startsWith("https://api.mailersend.com/")) {
        mailerSendBodies.push(JSON.parse(String(body)));
        if (mail === "rejected") {
          return new Response(JSON.stringify({ message: "Unprocessable" }), { status: 422 });
        }
        return new Response(null, { status: 202, headers: { "x-message-id": "msg_test_1" } });
      }
  ...
  return recorded as { mailerSendBodies: unknown[]; identityBodies: unknown[]; tokenCalls: number };
}
...
describe("staff password reset", () => {
  ...
  it("caches the access token across requests", async () => {
    ...
    expect(outbound.mailerSendBodies).toHaveLength(2);
  });
});
