-- Admin authentication — 14 PART 27.2.
--
...
-- unable to query outside it. A admin console exists to violate that on
-- purpose, because a support engineer needs to look at any customer's
-- workspace.
--
-- So admin are not a customer role with extra permissions. They are a separate
...
-- than a "never, except for admin" scattered through the same handlers.
--
-- Deliberately not Firebase, unlike customer auth (16 PART 30). Admin is a
...
-- exactly the kind of incident admin access exists for.
...
-- family, no "remember me". A stolen admin session is a far higher-value
...
-- Admin actions land in the same audit table as everything else, with
-- actor_type 'admin'. One trail rather than two: an investigation into what
