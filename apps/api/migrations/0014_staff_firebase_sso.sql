-- Staff sign in with Firebase, like everybody else. The database decides who
-- is an admin.
--
-- ── This reverses 0008, deliberately ───────────────────────────────────────
-- 0008 made staff auth independent of Firebase on the reasoning that "a
-- Firebase outage never stops AgentDisk's own team logging in to investigate
-- exactly the kind of incident staff access exists for". That reasoning was
...
-- staff credential and a customer credential are no longer structurally
-- incapable of reaching each other's routes. One Firebase token now reaches
-- both, and the ONLY thing separating them is the `staff_users` lookup below.
...
-- staff_sessions, because a Firebase ID token IS the session.
--
...
CREATE TABLE staff_users_new (
  ...
  -- the same person to Google and would otherwise be a different staff member
  ...
  -- deliberately not a foreign key for the same reason staff_actions has none:
...
-- keeps its id, email and role - so `staff_actions` rows written before this
-- migration still resolve to the same actor - and simply stops having a
-- password nobody will ever use again.
INSERT INTO staff_users_new (id, email, role, disabled_at, last_login_at, created_at, invited_by)
SELECT id, LOWER(email), role, disabled_at, last_login_at, created_at, NULL FROM staff_users;
...
INSERT OR IGNORE INTO staff_users (id, email, role, disabled_at, last_login_at, created_at, invited_by)
VALUES (
  'stf_seed_owner',
  'kernelv5@gmail.com',
  'super_admin',
