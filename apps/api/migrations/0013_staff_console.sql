-- The staff console's own schema — 32 PART 2, 7 and 9.
--
-- Five additions, and every one of them exists because a staff action needs a
...
--
-- `deleted_at` is set by DELETE /v1/staff/users/:id and cleared by the restore
...
-- index over those would be dead weight on every write. These serve the staff
...
-- *when*, and the window is the whole point: a staff member deleting somebody
...
-- ── staff_users: pending enrolment is a real state ─────────────────────────
--
-- TOTP is mandatory for every staff account, so a row can legitimately be
...
--
-- Backfilled to created_at rather than left NULL. Every staff account that
-- exists today was provisioned by scripts/provision-staff.mjs, which prints the
...
ALTER TABLE staff_users ADD COLUMN totp_confirmed_at INTEGER;
UPDATE staff_users SET totp_confirmed_at = created_at WHERE totp_confirmed_at IS NULL;

-- ── plans: which side last wrote, and when ─────────────────────────────────
--
-- Scalars, deliberately — there is no plan_sync_events table and should not be
-- one. The Sync History screen is a filter over staff_actions for `plan.%`,
...
--
-- 'inbound'  — Stripe -> D1 (a webhook, or the staff sync)
-- 'outbound' — D1 -> Stripe (a staff plan edit pushed through)
